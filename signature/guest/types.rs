...
impl EncodeSelf for Context {
    fn encode_self<'a, 'b>(
        ...
        e.encode_u32(self.digit)?;
        e.encode_array(self.pixels.len(), Kind::U32)?;
...
impl Decode for Context {
    fn decode(
        ...
        x.digit = d.decode_u32()?;
        let size_pixels = d.decode_array(Kind::U32)?;
        for _ in 0..size_pixels {
            x.pixels.push(d.decode_u32()?);
        }
        Ok(Some(x))
    }
}
